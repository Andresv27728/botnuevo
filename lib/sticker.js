...
// Note: This library requires FFmpeg to be installed on the system.
        ...
        exec(`ffmpeg -i ${tmpFileIn} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${tmpFileOut}`, (err) => {
            ...
            if (err) {
                ...
                if (err.message.includes('not found') || err.message.includes('ENOENT')) {
                    return reject(new Error('FFmpeg is not installed. Please install FFmpeg to use this command.'));
                }
                return reject(err);
            }
            resolve()
...
async function sticker(media, url, packname, author) {
    ...
    if (url) {
        ...
        media = await res.arrayBuffer().then(buffer => Buffer.from(buffer))
    }
